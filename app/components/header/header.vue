<template>
    ...
    <header id="site-header"
        ...
        <div class="container mx-auto flex items-center justify-between px-4">
            ...
            <nav>
                <nuxt-link to="/" class="hover:text-gray-600 mr-4">Home</nuxt-link>z
                <nuxt-link to="/about" class="hover:text-gray-600 mr-4">About</nuxt-link>
                <nuxt-link to="/contact" class="hover:text-gray-600">Contact</nuxt-link>
            </nav>
        </div>
    </header>
...
export default {
    setup() {  // Using Composition API
        ...
        const updateHeaderTop = () => {
            ...
            if (siteHeader) {
                siteHeader.style.top = topBarVisible.value ? `${topBarHeight.value}px` : '0px';
            }
        ...
        const adjustContentPadding = () => {
            ...
            if (mainContent) {
                const totalHeaderHeight = topBarVisible.value ? topBarHeight.value + headerHeight.value : headerHeight.value;
        ...
        const handleScroll = () => {
            ...
            if (scrollTop > lastScrollTop) { // Scrolling down
                if (scrollTop > headerHeight.value + topBarHeight.value && Math.abs(scrollTop - lastScrollTop) > tolerance) {
                    document.getElementById('site-header')?.classList.remove('translate-y-0');
                    document.getElementById('site-header')?.classList.add('-translate-y-full');
                }

                if (topBarVisible.value) {
                    topBarVisible.value = false; // using ref to update value
                    document.getElementById('top-bar')?.classList.add('-translate-y-full');
                ...
                if (scrollTop + window.innerHeight < document.documentElement.scrollHeight) {
                    if (Math.abs(scrollTop - lastScrollTop) > tolerance) {
                        document.getElementById('site-header')?.classList.remove('-translate-y-full');
                        document.getElementById('site-header')?.classList.add('translate-y-0');
                    }
                }

                if (scrollTop <= topBarHeight.value) {
                    topBarVisible.value = true;
                    document.getElementById('top-bar')?.classList.remove('-translate-y-full');
        ...
        onMounted(() => {
            ...
            if (siteHeader) {
                ...
                setTimeout(() => {  // Timeout to force re-rendering of the site-header for the transition to work.
